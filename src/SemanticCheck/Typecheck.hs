...
{- Insert list after first element -}
afterHead :: [a] -> [a] -> [a]
...
{- Tag a value with a position of another tagged value -}
infixl 9 @@
(@@) :: (PositionTag tb) => a -> tb -> AST.WithPosition a
(@@) = flip AST.WithPosition *. position
infixl 9 ?@@
{- Tag a value with a position of another tagged value, if it is auto-generated

  Use case: access modifier, static modifier, @Override annotation might be invalid, though not present in code
-}
(?@@) :: AST.WithPosition a -> AST.WithPosition b -> AST.WithPosition a
ta ?@@ tb = case position ta of
...
precheckClass cIass = do
  ...
  cIass & cconstructorsL %~ applyWhen' null (defConstructor cIass:)
      ...
      <&> cconstructorsL . traverse . crbodyL . stmtsL %~ applyWhen' (not . isThisCall . head) (afterHead $ genInitializers $ AST.cfields cIass)
  where precheckExtends = void . anchestors
        isThisCall (AST.ThisCall{}) = True
        isThisCall _                = False
        genInitializers = mapMaybe $ \field ->
          AST.finit field <&> \init ->
            AST.StmtOrExprAsStmt AST.AutoGenerated $ AST.Assign  (Just $ AST.This AST.AutoGenerated) (name field) init
...
checkMethod cIass method@AST.Method{ AST.moverride, AST.mbody } = do
  ...
  whenJust moverridden $ \(overCIass, overridden) -> do
    ...
    when (access method > access overridden)
      $ throwPretty ("Canont reduce visibility of inherited method " ++ name overCIass ++ "." ++ showSignature (name overridden) (ptypes overridden) ++ ".") $ position (taccess method ?@@ tname method)
  return TAST.Method {
    ...
    TAST.mbody = mbody' & stmtsTL %~ applyWhen (typee method == Void) (++[TAST.Return Nothing])
  }
...
checkConstructor cIass cr@AST.Constructor{ AST.crbody } = do
  ...
  return TAST.Constructor {
    ...
    TAST.crbody = crbody' & stmtsTL %~ (++[TAST.Return Nothing])
  }
