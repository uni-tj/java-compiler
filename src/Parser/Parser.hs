...
parseFieldDecl =     ((parseVisibility +.+ parseStatic +.+ parseType +.+ parseFieldName +.+ posLexem ASSIGN +.+ parseExpr +.+ posLexem SEMICOLON)
                        <<< (\(WithPosition visPos vis, (WithPosition stcPos stcFlg, (WithPosition tpePos tpe, (name, (_, (expr, (PositionedToken { position = endPos })))))))

                            ->  let correctSpan | visPos /= AutoGenerated = spanPos visPos endPos 
