...
codeGen :: [(Class, (CP.SearchFunctions, CP.ConstantPool))] -> [ClassFile]
codeGen classList = do
    let classFileObjects = map createClassFile classList

    classFileObjects

createClassFile :: (Class, (CP.SearchFunctions, CP.ConstantPool)) -> ClassFile
createClassFile (classInfo, (sf, cp)) = do
...
codeGenStmt (ThisCall className paras, localVarArr, sf) = do
    ...
    codeParas ++ [183] ++ (splitLenInTwoBytes $ (findConstructor sf) className (map fst paras))
...
codeGenExpr (FieldAccess fieldType expr classname static fieldName, localVarArr, sf) = do
    let (exprCode, t) = codeGenExpr (expr, localVarArr, sf)
    let getFieldCode = 180 : (splitLenInTwoBytes $ traceShow "0x1" $ (findField sf) classname fieldName fieldType)
